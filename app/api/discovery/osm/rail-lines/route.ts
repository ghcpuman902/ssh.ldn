...
export const GET = async (request: NextRequest) => {
  ...
  const radiusMeters = Number(
    request.nextUrl.searchParams.get("radiusMeters") ?? 6000
  );
  ...
  if (!Number.isFinite(radiusMeters) || radiusMeters < 100 || radiusMeters > 15000) {
  ...
  try {
    const geojson = await getOvergroundRailGeoJson({
      lat: parsed.lat,
      lng: parsed.lng,
      radiusMeters,
    });
    ...
    return Response.json(
      {
        error:
          error instanceof Error ? error.message : "OSM rail lines lookup failed",
      },
      { status: 502 }
    );
  }
