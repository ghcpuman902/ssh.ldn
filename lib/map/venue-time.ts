...
export type NightlifeAmenity =
  | "pub"
  | "bar"
  | "nightclub"
  | "music_venue";

export const NIGHTLIFE_AMENITIES: readonly NightlifeAmenity[] = [
  "pub",
  "bar",
  "nightclub",
  "music_venue",
...
export const isNightlifeAmenity = (
  amenity: string | null | undefined
): amenity is NightlifeAmenity =>
  amenity !== null &&
  amenity !== undefined &&
  NIGHTLIFE_AMENITIES.includes(amenity as NightlifeAmenity);

/** Base noise contribution weight by venue type (before time slot). */
export const AMENITY_BASE_WEIGHT: Record<NightlifeAmenity, number> = {
  pub: 0.5,
  bar: 0.7,
  nightclub: 1,
  music_venue: 0.9,
...
const resolveAmenity = (amenity: string | null | undefined): NightlifeAmenity =>
  isNightlifeAmenity(amenity) ? amenity : "pub";
...
const isOpenFromHours = (
  ...
  try {
    ...
    return oh.getState(nightStart) || oh.getState(probe) || oh.getState(nightEnd);
  } catch {
    return null;
  }
};
...
export const computeNightlifeScore = (
  features: Array<{
    amenity: string | null;
  ...
  const nightlifeFeatures = features.filter((feature) =>
    isNightlifeAmenity(feature.amenity)
  );

  const raw = nightlifeFeatures.reduce((sum, feature) => {
